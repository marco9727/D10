...
const dice = () => {
  const runNum = Math.floor(Math.random() * 6) + 1;
  console.log(runNum);
};
dice();
...
console.log(isThisAnEmail("mariorossi@libero.it"));
console.log(isThisAnEmail("mariorossilibero"));
...
console.log(howManyDays(new Date(1997, 10, 27)));
...
const isTodayMyBirthday = () => {
  const now = new Date();
  const myBirthday = new Date(1997, 10, 27);
  if (
    now.getFullYear() === myBirthday.getFullYear() &&
    now.getMonth() === myBirthday.getMonth() &&
    now.getDate() === myBirthday.getDate()
  ) {
    return true;
  } else {
    return false;
...
const myObj = { name: "Marco", age: 26, city: "Napoli" };
...
const onlyInLastMillennium = () => {
  return movies.filter(
    (movie) => parseInt(movie.Year) >= 1900 && parseInt(movie.Year) < 2000
  );
};
